...
if (consul.equals("1")){
	...
	conCent="SELECT ST_AsText(ST_Centroid(the_geom)) AS centroide_geom FROM cat_manz WHERE  (cve_ent||cve_mun||cve_loc||replace(cve_ageb,'-','')||cve_mza)=replace('"+clave+"','-','')";
	...
	conCent="SELECT ST_AsText(ST_Centroid(the_geom)) AS centroide_geom FROM cat_agebu ";
	conCent=conCent+" WHERE   (cve_ent||cve_mun||cve_loc||replace(cve_ageb,'-','')) = replace('"+clave+"','-','') ";
	conCent=conCent+" UNION ALL ";
	conCent=conCent+" SELECT ST_AsText(ST_Centroid(the_geom)) AS centroide_geom FROM cat_ageb "; 
	conCent=conCent+" WHERE (cve_ent||cve_mun||replace(cve_ageb,'-','')) = replace('"+clave+"',substring('"+clave+"',6,4),'') ";
	...
	conCent="SELECT ST_AsText(ST_Centroid(the_geom)) AS centroide_geom FROM cat_loc WHERE  (cve_ent||cve_mun||cve_loc)='"+clave+"'";
...
rs = str.executeQuery(conCent);
while(rs.next()){
punto = rs.getObject(1).toString();
}
...
//out.println (imagen); 
		...
		if(bandera=="1"){
			...
			String texto="href='http://dc046068asdggma.inegi.gob.mx:8070/openlayers2/index.jsp?sts=1&ban=1&st=AMBAS&capa=11&c1="+boxa[0]+"&c2="+boxa[1]+"&c3="+boxa[2]+"&c4="+boxa[3]+"&password="+(pasSigma)+"&cat=X"+"&punto="+(punto)+"'";
