...
function obt_geom(gid){
    var ajax=nuevoAjax();
    ajax.open("GET",'obtgeo_modcar_acu.jsp?id='+gid+'&ban=0&tipo=3', true);
...
if (filtipo.equals("X")){
  select1=" ";
}else if (filtipo.equals("1")){  //si procede por integracion (1),, por relacionada (2) y por proceso (3)
  select1=" proc in (1,2,3) and ";
...
if (filtipo2.equals("X")){
  select2=" ";
}else if (filtipo2.equals("1")){  //alta
  select2=" upper(left(tipo_act,4)) in ('ALTA','REAC') and ";
}else if (filtipo2.equals("2")){  //cambio
  select2="  upper(left(tipo_act,4)) not in ('ALTA','BAJA','REAC') and ";
}else if (filtipo2.equals("3")){   //baja
  select2=" upper(left(tipo_act,4)) in ('BAJA') and ";
}
...
if ((ent.equals("00"))){
         switch(regionalid){
          ...
          case "10":
            ...
            if (filent.equals("00")){
          consulta="select gid,cve_ent,cve_mun,cve_loc,cve_ageb,tipo_act as codigoact,f_registro,proc,fresp,case when geom is null then '0' else '1' end as geom,ban,cve_def,status,observ as nota,nom_loc,cgo_def,habitabil from act_cu.bcu_bd_loc_rur"+
            " where  "+select1+" "+select2+" upper(left(tipo_act,3))!='SIN' and f_registro between '"+filfe1+"' and '"+ filfe2 +"' "+filreg+
            " order by cve_ent,cve_mun,cve_loc,cve_ageb,gid";
          where = select1+" "+select2+" upper(left(tipo_act,3))!='SIN' and f_registro between '"+filfe1+"' and '"+ filfe2 +"' "+filreg+" order by cve_ent,cve_mun,cve_loc,cve_ageb,gid";
                }else{
          consulta="select gid,cve_ent,cve_mun,cve_loc,cve_ageb,tipo_act as codigoact,f_registro,proc,fresp,case when geom is null then '0' else '1' end as geom,ban,cve_def,status,observ as nota,nom_loc,cgo_def,habitabil from  act_cu.bcu_bd_loc_rur"+
              " where  "+select1+" "+select2+" upper(left(tipo_act,3))!='SIN' and f_registro between '"+filfe1+"' and '"+ filfe2 +"' and cve_ent = '"+filent+"'"+
            " order by cve_ent,cve_mun,cve_loc,cve_ageb,gid";
          where = select1+" "+select2+" upper(left(tipo_act,3))!='SIN' and f_registro between '"+filfe1+"' and '"+ filfe2 +"' and cve_ent = '"+filent+"' order by cve_ent,cve_mun,cve_loc,cve_ageb,gid";
                }
    ...
    consulta="select gid,cve_ent,cve_mun,cve_loc,cve_ageb,tipo_act as codigoact,f_registro,proc,fresp,case when geom is null then '0' else '1' end as geom,ban,cve_def,status,observ as nota,nom_loc,cgo_def,habitabil from  act_cu.bcu_bd_loc_rur"+
      " where  "+select1+" "+select2+"upper(left(tipo_act,3))!='SIN' and f_registro between '"+filfe1+"' and '"+ filfe2 +"' and cve_ent = '"+ent+"'"+
      " order by cve_ent,cve_mun,cve_loc,cve_ageb,gid";
    where = select1+" "+select2+"upper(left(tipo_act,3))!='SIN' and f_registro between '"+filfe1+"' and '"+ filfe2 +"' and cve_ent = '"+ent+"' order by cve_ent,cve_mun,cve_loc,cve_ageb,gid";
    select="<select name=filent class='boton'>";
  ...
  if(loc != null && !loc.isEmpty()){
    ...
    consulta="select gid,cve_ent,cve_mun,cve_loc,cve_ageb,tipo_act as codigoact,f_registro,proc,fresp,case when geom is null then '0' else '1' end as geom,ban,cve_def,status,observ as nota,nom_loc,cgo_def,habitabil from  act_cu.bcu_bd_loc_rur"+ " where upper(left(tipo_act,3))!='SIN' and  cve_ent||cve_mun||cve_loc||replace(cve_ageb,'-','') like '" + loc.replace("-", "") + "%' "+filult+" order by cve_ent,cve_mun,cve_loc,cve_ageb,gid";
 ...
 select2="<select name=filtipo2 class='boton' style='background: #aaffff'>";
       ...
       select2+="<option value='1' ";if (filtipo2.equals("7")){select2+="selected";}select2+=">ALTA</option>";
       select2+="<option value='2' ";if (filtipo2.equals("9")){select2+="selected";}select2+=">CAMBIO</option>";
     select2+="<option value='3' ";if (filtipo2.equals("8")){select2+="selected";}select2+=">BAJA</option></select>";
     //select2+="<option value='82' ";if (filtipo2.equals("82")){select2+="selected";}select2+=">82</option>";
	   //select2+="<option value='83' ";if (filtipo2.equals("83")){select2+="selected";}select2+=">83</option></select>";
      out.println( "<center class='t'><b class='t' style='background-color: #E7A4F9;'>&nbsp;&nbsp;Informacion procedente de la AC Integral&nbsp;&nbsp;(LOCALIDADES)</b><br><br>"
                + "<input class='boton'  type='button' value=' Manzanas ' onclick='window.open(\"modcar_acu.jsp?loc=&pass="+pass+"\",\"_self\");''>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                + "<input class='boton' type='button' value=' Pol. Externos ' onclick='window.open(\"modcar_acu_pe.jsp?loc=&pass="+pass+"\",\"_self\");''>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                + "<input class='boton' type='button' value=' Localidades ' disabled></center>"
                + "<form action=\"modcarloc_acu.jsp\" method=\"post\" name=\"enviar\">"
                + "<center><br>"
  ...
  out.println("<th>&nbsp;&nbsp;Id&nbsp;&nbsp;<th>&nbsp;&nbsp;Ver&nbsp;&nbsp;<th>&nbsp;&nbsp;Clave&nbsp;&nbsp;<th>&nbsp;Codigo&nbsp;<th colspan=1>&nbsp;Nombre&nbsp;<th>Fecha<br>&nbsp;registro&nbsp;<th>Fecha<br>&nbsp;respuesta&nbsp;<th>&nbsp;Procede&nbsp;<th>&nbsp;Respuesta&nbsp;<th title='Registro con informe'>&nbsp;<img src=images/res.png>&nbsp;");
  if (ban!=null){
   //out.println(consulta);
    ...
    while(rs.next()){
    //select gid,cve_ent,cve_mun,cve_loc,cve_ageb,tipo_act as codigoact,f_registro,proc,fresp,case when geom is null then '0' else '1' end as geom,ban,cve_def,status,observ as nota,nom_loc,cgo_def,habitabil
      ...
      if (rs.getObject(16) != null){
        cgo_def=rs.getObject(16).toString();
          ...
          String habita=rs.getObject(17).toString();
     ...
     switch(proc){
      ...
      if (geom.equals("1")){
        out.println("<td>"+gid+"<td align=center nowrap>&nbsp;<input type=button onclick='obt_geom(\""+gid+"\");' value=' Ver ' class='boton'>&nbsp;");
      }else{
        out.println("<td>"+gid+"<td align=center nowrap title='Sin geometria'>&nbsp;-&nbsp;");
            ...
            out.println("<td align=left nowrap title='"+cgotxt+"'>&nbsp;&nbsp;"+cgo+"&nbsp;&nbsp;");
            //out.println("<td align=center nowrap title='"+nota+"'>&nbsp;&nbsp;"+status+"&nbsp;&nbsp;");
            out.println("<td align=left nowrap title='"+nota+" - "+habita+"'>&nbsp;&nbsp;"+figura+"&nbsp;&nbsp;");
...
String barra="";
if(startIndex != 0) {
barra="<a class=liga1 href='modcarloc_acu.jsp?loc="+loc+"&pass="+pass+"&tipo="+tipo+"&ban="+ban+"&startIndex="+(startIndex-numRecordsPerPage)+"'> &nbsp;<<&nbsp; </a>";
}
...
for (n =0 ; n<numPages; n++){
  if (n*numRecordsPerPage == startIndex) {
    ...
    barra+="<a class=liga1 href='modcarloc_acu.jsp?filent="+filent+"&filtipo="+filtipo+"&filtipo2="+filtipo2+"&filfe1="+filfe1+"&filfe2="+filfe2+"&loc="+loc+"&pass="+pass+"&tipo="+tipo+"&ban="+ban+"&startIndex="+(numRecordsPerPage*n)+"'> &nbsp;"+(n+1)+"&nbsp; </a>";
  }
 }
...
if(startIndex+numRecordsPerPage <= totalRows){
  barra+="<a class=liga1 href='modcarloc_acu.jsp?filent="+filent+"&filtipo="+filtipo+"&filtipo2="+filtipo2+"&filfe1="+filfe1+"&filfe2="+filfe2+"&loc="+loc+"&pass="+pass+"&tipo="+tipo+"&ban="+ban+"&startIndex="+(startIndex+numRecordsPerPage)+"'> &nbsp;>>&nbsp; </a>";
}
