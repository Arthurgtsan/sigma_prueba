...
ventana = window.open('verobsmzval.jsp?gid='+gid+'&tipo='+tipo+'&tipote='+tipot+'&nivel='+nivel,'Observapre','scrollbars=no,toolbar=no');
ventana.focus();
}
...
consulta="select oid,cgo_act,fact,tipo_ant,clave1,mov_ant,tipo_act,clave2,mov_act,obs,vreg,voc,motivo,"+
        " (select oid from v_te_mza t1 where t1.clave1=t2.clave1 limit 1) as max1,"+
        " (select oid from v_te_mza t1 where t1.clave2=t2.clave1 limit 1) as max2,"+
        " img,usuario,fdg,correo,tipte,obsoc,obsre,obsed,jpg from v_te_mza_forma t2 ";
...
if ((ent.equals("00"))){   //NACIONAL
     switch(regionalid){
          ...
          consulta="select oid,cgo_act,fact,tipo_ant,clave1,mov_ant,tipo_act,clave2,mov_act,obs,vreg,voc,motivo,"+
          " (select oid from v_te_mza t1 where t1.clave1=t2.clave1 limit 1) as max1,"+
        " (select oid from v_te_mza t1 where t1.clave2=t2.clave1 limit 1) as max2,img,usuario,fdg,correo,tipte,obsoc,obsre,obsed,jpg "+
          "from v_te_mza_forma t2"+
  ...
  if(loc != null && !loc.isEmpty()){
       ...
       */
       ...
       select1+="<option value='F1' ";if (filcgo.equals("F1")){select1+="selected";}select1+=">F1-Forma</option>";
       select1+="<option value='F2' ";if (filcgo.equals("F2")){select1+="selected";}select1+=">F2-Posicion</option>";
       select1+="<option value='F3' ";if (filcgo.equals("F3")){select1+="selected";}select1+=">F3-Dimension</option></select>";
      out.println( "<center class='t'>Tabla de Cambios de forma de Manzanas</center>"
              	+ "<form action=\"temf.jsp\" method=\"post\" name=\"enviar\">"
      ...
      String checkerr="<table border=0><tr class=n><td align=center rowspan=2><input class='boton' type='checkbox' name='filerr' ";
       if (filerr!=null && filerr.equals("on") ){
        checkerr+="checked";
      }
//       checkerr+=">Solo con Obs&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<td align=center>Val OC >>> ";
...
*/
   ...
   //if (s1!=2){
    ...
    //}
     ...
     out.println("<th rowspan=2>Obs<br>&nbsp;Reg&nbsp;");
  ...
  */
  out.println("<th rowspan=2>&nbsp;&nbsp;CGO&nbsp;&nbsp;<th rowspan=2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Fecha/Hora&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<th rowspan=2>&nbsp;Obs&nbsp;<!--<th rowspan=2>&nbsp;Motivo&nbsp;--><th colspan=4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Anterior&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<th colspan=4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Actual&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
        out.println("<th rowspan=2><img title='Subida de Arcgis' src=images/arcgis.png>");
...
//out.println(consulta);
  if (ban!=null){
    String consultacount=consulta.replace("oid,cgo_act,fact,tipo_ant,clave1,mov_ant,tipo_act,clave2,mov_act,obs,vreg,voc,motivo, (select oid from v_te_mza t1 where t1.clave1=t2.clave1 limit 1) as max1, (select oid from v_te_mza t1 where t1.clave2=t2.clave1 limit 1) as max2,img,usuario,fdg,correo,tipte,obsoc,obsre,obsed,jpg","count(*)");
    consultacount=consultacount.replace("order by oid","");
    String consultaexp=consulta.replace("oid,cgo_act,fact,tipo_ant,clave1,mov_ant,tipo_act,clave2,mov_act,obs,vreg,voc,motivo, (select oid from v_te_mza t1 where t1.clave1=t2.clave1 limit 1) as max1, (select oid from v_te_mza t1 where t1.clave2=t2.clave1 limit 1) as max2,img,usuario,fdg,correo,tipte,obsoc,obsre,obsed,jpg","cgo_act,fact,tipo_ant,clave1,mov_ant,tipo_act,clave2,mov_act,obs,vreg,voc,motivo,img,usuario,fdg,correo,tipte,obsoc,obsre,obsed,jpg");
...
/*
  if (s1==2){
    ...
    out.println ("<td "+col+" align=center nowrap title='"+ocobs+"'>&nbsp;&nbsp;<input type=button onclick='observaval("+gid+",\"O\","+s1+",\""+tipte+"\");' value=' Ver ' class='boton'>&nbsp;&nbsp;");
...
/*
    ...
    out.println ("<td "+col+" align=center nowrap title='"+reobs+"'>&nbsp;&nbsp;<input type=button onclick='observaval("+gid+",\"R\","+s1+",\""+tipte+"\");' value=' Ver ' class='boton'>&nbsp;&nbsp;");
    col="";
    if (edobs.length()>1){
      col="bgcolor=#2EFE2E";
    }
    ...
    out.println ("<td "+col+" align=center nowrap title='"+edobs+"'>&nbsp;&nbsp;<input type=button onclick='observaval("+gid+",\"E\","+s1+",\""+tipte+"\");' value=' Ver ' class='boton'>&nbsp;&nbsp;");
...
*/
    ...
    if ((cons==44 || cons==60 || cons==234 || cons==496 || cons==1705) && (cgo.length()!=2)) { //cambia datos de la tabla de eq
      ...
      if (mov2.equals("0")){
            ...
            if (img.equals("00")){   //aqui pongo un icono si viene desde arcgis
                out.println("<td align=center nowrap><img width=80% title='Subida de Arcgis' src='images/arcgis.png'>");
          }else{
              out.println("<td>&nbsp;");
          }
                      ...
                      int gidint = Integer.parseInt(gid);
                        ...
                        //}else if (valof.equals("R")){
                          ...
                          //if (gidint>=max1 && gidint>=max2){
                            if (max1==0 && max2==0 && !(user.equals("1"))){   //no hay otro registro en la  tabla de la TE   user=1 es del proceso de cambios de frentes ,, no pueden regresar su manzana original
                					 ...
                					 out.println("<td align=center nowrap><div id=c12"+gid+">&nbsp;&nbsp;<img title='Existe un registro de TE con la misma clave' src='images/otroab.png'>&nbsp;&nbsp;</div>");
                				  }
                        }
...
String barra="";
if(startIndex != 0) {
barra="<a class=liga1 href='temf.jsp?filerr1="+filerr1+"&filerr2="+filerr2+"&filerr="+filerr+"&filent="+filent+"&filtipo="+filtipo+"&filcgo="+filcgo+"&filfe1="+filfe1.substring(0,10)+"&filfe2="+filfe2.substring(0,10)+"&loc="+loc+"&pass="+pass+"&ban="+ban+"&startIndex="+(startIndex-numRecordsPerPage)+"'> &nbsp;<<&nbsp; </a>";
}
...
for (n =0 ; n<numPages; n++){
  if (n*numRecordsPerPage == startIndex) {
    ...
    barra+="<a class=liga1 href='temf.jsp?filerr1="+filerr1+"&filerr2="+filerr2+"&filerr="+filerr+"&filent="+filent+"&filtipo="+filtipo+"&filcgo="+filcgo+"&filfe1="+filfe1.substring(0,10)+"&filfe2="+filfe2.substring(0,10)+"&loc="+loc+"&pass="+pass+"&ban="+ban+"&startIndex="+(numRecordsPerPage*n)+"'> &nbsp;"+(n+1)+"&nbsp; </a>";
  }
 }
...
out.println(barra+"<br><br>");
 ...
 String export = consultaexp.replace("%", "$");
        ...
        export = export.replace("v_te_mza_forma","v_te_mza_exportar_forma");
