...
//try {
	...
	while(rs.next()){
		...
		f_registro = rs.getObject(8).toString();
	...
	if (n==0){
		...
		out.println("<br><center class=c><table><tr><td bgcolor='#f5a9e1' class=c>Caserio viene desde <u><b>* CUR *</b></u></table></center>");
		//if (Integer.parseInt(cve_mza)!=800 && status!=7){
			out.println("<br><center class=n><b>--> SI ES MANZANA, AGREGARLAS DESDE LA HERRAMIENTA DE MANZANAS <--</b></center>");
     	//}
		...
		if (proc==1){
			...
			out.println("<tr class=n><td colspan=10 align=center><font color=red>Se pueden crear caserios desde manzanas cuando no cumplen los criterios de Mz</font>");
		}
...
if (proc==1 ){ // si ya esta atendido no pongo los botones
...
/*
	String addsap="";
    	String txtrel="";
    	if (cgo.equals("46") || proc==1 || (Integer.parseInt(cve_mza)!=800 && status!=7)){
    		addsap="disabled";
    	}else{
    		if (cgo_def==1){
