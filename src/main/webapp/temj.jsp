...
function modval(val,cons,id,cgo,tipoval,tipote) {
    var ajax=nuevoAjax();
    ajax.open("GET",'modvalmzJ.jsp?cons='+cons+'&id='+id+'&tipoval='+tipoval+'&val='+val+'&tipote='+tipote+'&cgo='+cgo, true);
...
ventana = window.open('tem_cambiaJ.jsp?ban=0&tipote='+tipote+'&i='+gid,'CambiaTEM','scrollbars=no,toolbar=no');
ventana.focus();
...
function regresa(tipo,gid,tipote) {
  document.all.pleasewaitScreen.style.visibility="visible";
    var ajax=nuevoAjax();
    if (tipote.substr(14,16)=='800'){
      tipo="2";
    }else{
      tipo="1";   //caserio
    }
        ajax.open("GET",'regresamzJ.jsp?tipo='+tipo+'&gid='+gid, true);
    ajax.onreadystatechange=function()
    {
        if(ajax.readyState==4)  {
...
ventana = window.open('verobsmzval.jsp?gid='+gid+'&tipo='+tipo+'&tipote=4&nivel='+nivel,'Observapre','scrollbars=no,toolbar=no');
ventana.focus();
}
...
consulta="select oid,cgo_act,fact,tipo_ant,clave1,mov_ant,tipo_act,clave2,mov_act,obs,vreg,voc,motivo,"+
        ...
        " img,usuario,fdg,correo,tipte,obsoc,obsre,obsed,jpg from v_te_mza_j t2 ";
...
if ((ent.equals("00"))){   //NACIONAL
  ...
  if(loc != null && !loc.isEmpty()){
       ...
       select2+="<option value='2' ";if (filtipo.equals("2")){select2+="selected";}select2+=">GEOMETRIA</option></select>";
       ...
       select1+="<option value='2' ";if (filtipo.equals("2")){select1+="selected";}select1+=">GEOMETRIA</option></select>";

       select1+="&nbsp;&nbsp;&nbsp;&nbsp;Cgo:&nbsp;<select name=filcgo class='boton'>";
       select1+="<option value='J' ";if (filcgo.equals("J")){select1+="selected";}select1+=">J</option></select>";
      out.println( "<center class='t'>Tabla de Equivalencia de Manzanas, CAMBIO DE AMBITO</center>"
              	+ "<form action=\"temj.jsp\" method=\"post\" name=\"enviar\">"
...
////////validacion regional
    ...
    out.println ("<td "+col+" align=center nowrap title='"+edobs+"'>&nbsp;&nbsp;<input type=button onclick='observaval("+gid+",\"E\","+s1+",\""+tipte+"\");' value=' Ver ' class='boton'>&nbsp;&nbsp;");
         /* if ((cons==44 || cons==60 || cons==234 || cons==496 || cons==1705) && (cgo.length()!=2)) { //cambia datos de la tabla de eq
            out.println("<td align=center nowrap title='"+cgotxt+"'><div id=c1"+gid+">&nbsp;&nbsp;<b><font color=red onclick='edita("+gid+",\""+tipte+"\")'>"+cgo+"</font></b>&nbsp;&nbsp;</div>");
          }else{*/
    ...
    if ((cons==44 || cons==60 || cons==234 || cons==496 || cons==1705) && (cgo.length()!=2)) { //cambia datos de la tabla de eq
      ...
      int gidint = Integer.parseInt(gid);
        ...
        if (valof.equals("S") || (!(cons==44 || cons==60 || cons==234 || cons==496 || cons==1705))){
        ...
        //}else if (valof.equals("R")){
          ...
          if (gidint>=max1 && gidint>=max2){
           out.println("<td align=center nowrap><div id=c12"+gid+">&nbsp;&nbsp;<img title='Eliminar registro de TE' src=images/borra.gif onclick='regresa(\""+cgo+"\","+gid+",\""+clave2+"\")';>  &nbsp;&nbsp;</div>");
          }else{
...
String barra="";
if(startIndex != 0) {
barra="<a class=liga1 href='temj.jsp?filerr1="+filerr1+"&filerr2="+filerr2+"&filerr="+filerr+"&filent="+filent+"&filtipo="+filtipo+"&filcgo="+filcgo+"&filfe1="+filfe1.substring(0,10)+"&filfe2="+filfe2.substring(0,10)+"&loc="+loc+"&pass="+pass+"&ban="+ban+"&startIndex="+(startIndex-numRecordsPerPage)+"'> &nbsp;<<&nbsp; </a>";
}
...
for (n =0 ; n<numPages; n++){
  if (n*numRecordsPerPage == startIndex) {
    ...
    barra+="<a class=liga1 href='temj.jsp?filerr1="+filerr1+"&filerr2="+filerr2+"&filerr="+filerr+"&filent="+filent+"&filtipo="+filtipo+"&filcgo="+filcgo+"&filfe1="+filfe1.substring(0,10)+"&filfe2="+filfe2.substring(0,10)+"&loc="+loc+"&pass="+pass+"&ban="+ban+"&startIndex="+(numRecordsPerPage*n)+"'> &nbsp;"+(n+1)+"&nbsp; </a>";
  }
 }
...
out.println(barra+"<br><br>");
 ...
 String export = consultaexp.replace("%", "$");
        ...
        export = export.replace("v_te_mza_j","v_te_mza_exportar_j");
