<%@ page import="java.util.*" session="true" %>
<%@ page import="java.sql.*"%>
<%
String  id = request.getParameter("id");
String  ban = request.getParameter("ban");
String tipo = request.getParameter("tipo");
String consulta="";
HttpSession sesion = request.getSession(false);
String remotehost  = session.getAttribute("remotehost").toString();
String base="";
try {
  /*if (tipo.equals("1")){
        consulta = "select ST_AsText(st_transform(geom,3857)) from vw_carga_tcar where clave_censor||cvegeo='"+id+"'";
  }else{
        consulta = "select ST_AsText(st_transform(geom,3857)) from vw_carga_tcar_masivo where clave_censor||cvegeo='"+id+"'";
}*/
     consulta = "select ST_AsText(st_transform(geom,3857)) from vw_carga_tcar_masivo where cveoper_censor||cvegeo='"+id+"'";

      ResultSet rs = null;
      Statement str = null;
      Connection conexion = null;
      Class.forName("org.postgresql.Driver");
      String hostbd  = session.getAttribute("hostbd").toString();
      String remotehostbd  = session.getAttribute("remotehostbd").toString();
      conexion = DriverManager.getConnection("jdbc:postgresql://10.153.3.25:5434/actcargeo10","arcgis","arcgis");
    str = conexion.createStatement(rs.TYPE_SCROLL_SENSITIVE, rs.CONCUR_UPDATABLE);
    rs = str.executeQuery( consulta );
	  rs.next();
    out.print(rs.getObject(1).toString());
    str.close();
    conexion.close();
    }
    catch (SQLException ex){
      out.print("1");
  }
      catch(Exception ex){
        out.print("1");
   }
%>
