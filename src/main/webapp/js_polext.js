...
function onFeatureSelectPe(event,feat){
...
var theHTMLextra ='';
var theHTMLextra2 ='';
if (butagmzpre.active || drawmz.active || drawmulti.active || (modiatmz.active && feature.state == OpenLayers.State.INSERT) || (modiatmz.active && (feature.attributes.ban==2 || feature.attributes.ban==0)) ){
    if  (drawmz.active || butagmzpre.active){
...
theHTML += '<input onclick="updateAttributesPe(\'' + feature.id + '\');" class=input type="button" name="enviar" value="Guardar">';
if(!(modiatmz.active)){
    theHTML += '&nbsp;&nbsp;&nbsp;<input onclick="crearHuecoPe(\'' + feature.id + '\');" class=input type="button" name="enviar" value="Hueco">';
}
    ...
    ("pop", varcent, new OpenLayers.Size(280,150),
    theHTML,    null,   true    //este false es el boton de cerrar
       ,onPopupClosepeloc               //funcion al cerrar
    );
   ...
   alertmsg("No puede cambiar de clave el poligono externo existente")
   return false;
}
}//funcion
...
function updateAttributesPe(id){
...
if(feature.state != OpenLayers.State.INSERT) {
    ...
    if (cve_pe_tipo==1){
            feature.attributes.obsoc=cve_pe_temp;
            feature.attributes.err=1;
    }
...
function crearHuecoPe(id){
    ...
    if (sal==0){
            alertmsg("Debe de hacerlo sobre poligonos externo");
             if(feature.fid == undefined) {
                wfs.destroyFeatures([feature]);
            } else {
                feature.state = OpenLayers.State.DELETE;
...
function onFeatureUnSelectPe(event){
var feature = event.feature;
    if (feature.modified){
        alertmsg("Movido");
        feature.attributes.img  = baseimg;
...
var DeleteFeaturePe = OpenLayers.Class(OpenLayers.Control, {
    ...
    clickFeature: function(feature) {
                if (revisa_idmax_noborraPe(feature.fid)){
                    alertmsg ("No se puede borrar una poligono externo agregado");
                    return false;
                }
...
function revisa_idmax_noborraPe(gid){
    if ((parseInt(gid.substring(8,gid.length))>max_pe || gid.substring(0,1)=='O')){
        return true;
    }else{
        return false;
    }
...
function obtCvePeloc(feature){
    ...
    ajax.open("GET",'obtcve.jsp?tipo=pe&geom='+geom+'&clave='+cveconca, true);
...
function onPopupClosepeloc(){
    var feature = wfs.getFeatureBy('id', idpop);
    if (feature!=null){
        if (feature.popup!=null){
            map.removePopup(feature.popup);
        }
        if (drawmz.active || drawmulti.active || butagmzpre.active){
            wfs.destroyFeatures([feature]);
        }
    }
}
