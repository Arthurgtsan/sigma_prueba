...
function obt_geom(gid,ban){
    var ajax=nuevoAjax();
    ajax.open("GET",'obtgeo_modcar.jsp?id='+gid+'&ban='+ban+'&tipo=1', true);
...
ventana = window.open('verrespmcc.jsp?gid='+gid+'&ban='+ban+'&figura='+figura+'&clave='+clave,'Respuesta','scrollbars=no,toolbar=no');
ventana.focus();
}
...
if (filtipo.equals("X")){
  ...
  select1=" (proc=0 and (cgo||cve_mza!='5800') and ((figura='CENSOR' and status in ('1','7')) or figura='TCAR')) and  ";
}else if (filtipo.equals("1")){
  select1=" (proc=1 or proc=2) and ";
}else if (filtipo.equals("2")){
  select1=" ((cgo||cve_mza not in ('5800','',''))) and (proc>2 or status in (3,4,5,6,8,9)) and";
}else if (filtipo.equals("3")){    //proyectadas
select1=" cgo||cve_mza='5800' and ";
}

if (filcgo.equals("X")){
  select1+="";
}else {
  select1+=" cgo='"+filcgo+"' and ";
}
/*
...
if (filtipo2.equals("X")){
  ...
  select2=" status in (1,7) and  ";
}else if (filtipo2.equals("2")){
  select2=" status=2 and ";
}else if (filtipo2.equals("3")){
  select2=" status in (3,4,5,6,8,9) and";
}
...
if ((ent.equals("00"))){
         switch(regionalid){
          ...
          case "10":
            ...
            if (filent.equals("00")){
          consulta="select gid,cve_ent,cve_mun,cve_loc,cve_ageb,cve_mza,tipomza,cgo,f_registro,proc,fresp,case when geom is null then '0' else '1' end as geom,ban,cve_def,tipo,status,nota,figura,cgo_def,mal,tipo_figura,cveoper from mcc_poblacion.cat_manz_modcar"+
  ...
  if(loc != null && !loc.isEmpty()){
    ...
    consulta="select gid,cve_ent,cve_mun,cve_loc,cve_ageb,cve_mza,tipomza,cgo,f_registro,proc,fresp,case when geom is null then '0' else '1' end as geom,ban,cve_def,tipo,status,nota,figura,cgo_def,mal,tipo_figura,cveoper from mcc_poblacion.cat_manz_modcar"+ " where   ((figura='CENSOR' and status!=2) or (figura='TCAR')) and (cve_ent||cve_mun||cve_loc||replace(cve_ageb,'-','')||cve_mza like '" + loc.replace("-", "") + "%' "+filult+" or cve_def like '" + loc.replace("-", "") + "%') order by cve_ent,cve_mun,cve_loc,cve_ageb,cve_mza,gid";
       ...
       select1+="<option value='1' ";if (filtipo.equals("1")){select1+="selected";}select1+=">CON RESPUESTA</option>";
       select1+="<option value='2' ";if (filtipo.equals("2")){select1+="selected";}select1+=">NO PROCEDE</option>";
       //select1+="<option style='background: #aaffff' value='3' ";if (filtipo.equals("3")){select1+="selected";}select1+=">ANALIZAR</option></select>";
       select1+="</select>";
       ...
       select2+="<option value='2' ";if (filcgo.equals("2")){select2+="selected";}select2+=">SUBDIV DE MZ</option>";
       ...
       select2+="<option value='5' ";if (filcgo.equals("5")){select2+="selected";}select2+=">CAMBIO DE MZ</option>";
       select2+="<option value='6' ";if (filcgo.equals("6")){select2+="selected";}select2+=">APERTURA VIAL</option>";
       select2+="<option value='44' ";if (filcgo.equals("44")){select2+="selected";}select2+=">ALTA DE CD</option>";
       select2+="<option value='45' ";if (filcgo.equals("45")){select2+="selected";}select2+=">BAJA DE CD</option>";
       select2+="<option value='46' ";if (filcgo.equals("46")){select2+="selected";}select2+=">MOVIMIENTO DE CD</option>";
       select2+="<option value='FR' ";if (filcgo.equals("FR")){select2+="selected";}select2+=">CAMBIO DE FRENTES</option>";
       select2+="</select>";


       select3="<select name=filtipo2 class='boton'>";
       select3+="<option value='X' ";if (filtipo2.equals("X")){select3+="selected";}select3+=">TODOS</option>";
       select3+="<option value='1' ";if (filtipo2.equals("1")){select3+="selected";}select3+=">ACEPTADO</option>";
       select3+="<option value='2' ";if (filtipo2.equals("2")){select3+="selected";}select3+=">CAMBIOS</option>";
       select3+="<option value='3' ";if (filtipo2.equals("3")){select3+="selected";}select3+=">RECHAZADO</option>";
       select3+="</select>";

      out.println( "<center class='t'><b  style='background-color: #F5A560;' >&nbsp;&nbsp;Informacion procedente del Modulo Cartografico EIC2025 (MZ/CD)&nbsp;&nbsp;</b><br><br>"
                + "<input class='boton' disabled type='button' value=' Manzanas '>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                + "<input class='boton' type='button' value=' Localidades ' onclick='window.open(\"modcarloc.jsp?loc=&pass="+pass+"\",\"_self\");''></center>"
                + "<form action=\"modcar.jsp\" method=\"post\" name=\"enviar\">"
                + "<center><br>"
                ...
                + "&nbsp;&nbsp;<font class=n>Status:</font>&nbsp;"+select3);
/*  //deshabilito la opcion del FR
if (filtipo2.equals("on")){
...
*/
  ...
  out.println("<th>&nbsp;&nbsp;Ver&nbsp;&nbsp;<th>&nbsp;&nbsp;Clave Oper&nbsp;&nbsp;<th>&nbsp;&nbsp;Clave Geo&nbsp;&nbsp;<th>Tipo&nbsp;de<br>&nbsp;manzana&nbsp;<th>&nbsp;Codigo&nbsp;<th colspan=2>&nbsp;Status/MCC&nbsp;<th>Fecha<br>&nbsp;registro&nbsp;<th>Fecha<br>&nbsp;respuesta&nbsp;<th>&nbsp;Procede&nbsp;<th>&nbsp;Respuesta&nbsp;<th title='Registro con informe'>&nbsp;<img src=images/res.png>&nbsp;");
  ...
  if (ban!=null){
    ...
    //gid,cve_ent,cve_mun,cve_loc,cve_ageb,cve_mza,tipomza,cgo,f_registro,proc,fresp,geom
      ...
      if (rs.getObject(17) != null){
        nota=rs.getObject(17).toString();
       }
      String figura=rs.getObject(18).toString();
       String cgo_def="";
      if (rs.getObject(19) != null){
        cgo_def=rs.getObject(19).toString();
      }
      int mal = Integer.parseInt(rs.getString(20));
      
      //int tipofig = Integer.parseInt(rs.getString(21));
      cveoper = rs.getString(22);
      /*
      switch (tipofig){
        case 1: tipo_figura="SE";break;
        case 3: tipo_figura="RA";break;
        case 2: tipo_figura="TCAR";break;
        case 8: tipo_figura="CM";break;
    }*/
      ...
      switch(cgo){
             ...
             case "5":
                      cgotxt="Cambio de forma de mza";
                  if (cve_mza.equals("800")){
                      sttxt="Analizar";
                  }
                  break;
      ...
      switch(proc){
        ...
        case "1":
          protxt="<td align=center nowrap><div id=pr1"+gid+cgo+">&nbsp;&nbsp;<font color='green'><b>SI PROCEDE</b></font>&nbsp;&nbsp;</div>";
            resp="<td align=center nowrap><div id=pr2"+gid+cgo+">&nbsp;<input type=button onclick='ver_resp(\""+cve_ent+" "+cve_mun+" "+cve_loc+" "+cve_ageb+" "+cve_mza+"\","+gid+",\""+cgo+"\",\""+figura+"\",);' value=' Respuesta ' class='boton'>&nbsp;</div>";
          break;
        case "2":
          protxt="<td align=center nowrap><div id=pr1"+gid+cgo+">&nbsp;&nbsp;<font color='green'><b>SI PROCEDE</b></font>&nbsp;&nbsp;</div>";
           if (cgo_def.equals("1")){
      ...
      //if (((status.equals("3") || status.equals("4") || status.equals("5") || status.equals("6") || status.equals("8") || status.equals("9")) && (!cve_mza.equals("800")))){
        if (((status.equals("3") || status.equals("4") || status.equals("5") || status.equals("6") || status.equals("8") || status.equals("9")) )){
            protxt="<td align=center nowrap><div id=pr1"+gid+cgo+">&nbsp;&nbsp;<font color='red'><b>NO PROCEDE</b></font>&nbsp;&nbsp;</div>";
            //resp="<td align=center nowrap><div id=pr2"+gid+cgo+">&nbsp;&nbsp;--&nbsp;&nbsp;</div>";
      }else if (cve_mza.equals("800") && cgo.equals("5")){
      ...
      if (geom.equals("1")){
        out.println("<td align=center nowrap>&nbsp;<input type=button onclick='obt_geom(\""+gid+figura+"\",\""+cgo+"\");' value=' Ver ' class='boton'>&nbsp;");
      }else{
        out.println("<td align=center nowrap title='Sin geometria'>&nbsp;-&nbsp;");
      }
            out.println("<td align=center nowrap>&nbsp;"+cveoper+"<td align=center nowrap>&nbsp;"+cve_ent+"&nbsp;"+cve_mun+"&nbsp;"+cve_loc+"&nbsp;"+cve_ageb+"&nbsp;"+cve_mza+"&nbsp;");
            ...
            out.println("<td align=left nowrap title='"+nota+"'>&nbsp;&nbsp;"+sttxt+"&nbsp;&nbsp;");

            if (!(nota.equals(""))){
              out.println("<img src=images/nota.png align='right'>");
            }
...
String barra="";
if(startIndex != 0) {
barra="<a class=liga1 href='modcar.jsp?loc="+loc+"&pass="+pass+"&tipo="+tipo+"&ban="+ban+"&startIndex="+(startIndex-numRecordsPerPage)+"'> &nbsp;<<&nbsp; </a>";
}
...
for (n =0 ; n<numPages; n++){
  if (n*numRecordsPerPage == startIndex) {
    ...
    barra+="<a class=liga1 href='modcar.jsp?filent="+filent+"&filtipo="+filtipo+"&filcgo="+filcgo+"&filtipo2="+filtipo2+"&filfe1="+filfe1+"&filfe2="+filfe2+"&loc="+loc+"&pass="+pass+"&tipo="+tipo+"&ban="+ban+"&startIndex="+(numRecordsPerPage*n)+"'> &nbsp;"+(n+1)+"&nbsp; </a>";
  }
 }
...
if(startIndex+numRecordsPerPage <= totalRows){
  barra+="<a class=liga1 href='modcar.jsp?filent="+filent+"&filtipo="+filtipo+"&filcgo="+filcgo+"&filtipo2="+filtipo2+"&filfe1="+filfe1+"&filfe2="+filfe2+"&loc="+loc+"&pass="+pass+"&tipo="+tipo+"&ban="+ban+"&startIndex="+(startIndex+numRecordsPerPage)+"'> &nbsp;>>&nbsp; </a>";
}
out.println(barra+"<br><br>");
 String export = consulta.replace("%", "$");
  ...
  //out.println("</table><br>ERROR FATAL<br><br>"+consulta);
}
